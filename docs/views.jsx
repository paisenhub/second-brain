// Views — Home, Reminders, Knowledge, Profile (static/client-side version)
...
function HomeView({ memories, addMemory, user }) {
  ...
  const firstName = user ? (user.name || 'Sam').split(' ')[0] : 'Sam';
              ...
              {!aiResponse.summary && !window.BRAIN.getKey() && (
                <div className="ai-summary" style={{ borderColor: 'var(--amber)' }}>
                  <div className="ai-summary-label" style={{ color: 'var(--amber)' }}>
                    <span className="ai-dot" style={{width:10,height:10,background:'var(--amber)'}}></span>
                    Add API key for AI summaries
                  </div>
                  <p style={{ color: 'var(--muted)' }}>
                    Click the settings icon (⚙) in the top bar to add your free Gemini API key and get AI-powered answers.
                  </p>
                </div>
              )}
              {aiResponse.results.length > 0 && (
...
function RemindersView({ events }) {
  ...
  const pxPerHour = 64;
  const nowOffset = ((10 + 24/60) - startHour) * pxPerHour;
  ...
  return (
    <div className="page">
      ...
      <div className="know-stats">
        <div className="stat"><div className="label">Total memories</div><div className="val">{memories.length}</div><div className="delta">+12 this week</div></div>
...
// ---------- PROFILE ----------
function ProfileView({ user = {}, onLogout, onSettings }) {
      ...
      <div className="profile-grid">
        <div>
          <div className="profile-card">
            <div className="avatar-lg">{(user.initials || (user.name || 'S')[0]).slice(0,2)}</div>
            ...
            {onSettings && (
              <button onClick={onSettings} className="btn-ghost"
                style={{ width: '100%', justifyContent: 'center', marginTop: 14, fontSize: 12, color: hasKey ? 'var(--mint)' : 'var(--amber)' }}>
        ...
        <div>
          ...
          <div className="tile" style={{ marginTop: 16 }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 13, margin: '0 0 14px' }}>
              Things from past-you that present-you might want to see again. Brain 1.0 picks 3 each morning.
            </p>
